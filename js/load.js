...
Game.Boot.prototype = {
  ...
  create: function () {
    ...
    // -- ------------------------------------------------------------------------------------------
	this.buttonContinue = this.add.button(1,0, 'button-inst', this.instrucao, this);
	this.buttonContinue.input.useHandCursor = true;
	
	// -- Verificacao se no armazenamento local nao existe nehum registo, caso aconteça sera considerado que nao houve nenhum jogo, considerando a pontuacao com 0
...
Game.Over.prototype = {
  ...
  // -- XMLHttpRequest é uma API disponível em linguagens de script para navegadores web tais como JavaScript. É utilizada para enviar requisições HTTP ou HTTPS
  //diretamente para um servidor web e carregar os dados de resposta do servidor diretamente de volta ao script, com utilizacao de AJAX, no qual este tem como
  //objetivo principal efetuar pedidos ao servidor sem ter de recarregar a pagina
	...
	// -- Enviar atraves do metodo "Post" a pontuacao adquirida no fim de um jogo para o ficheiro php "updateScore.php"
    xhttp.open( "POST", "/phaser/Jogo/api/updateScore.php", true );
	
	// -- Tipo de conteudo nao e texto, portanto tenho de configurar para que se possa enviar a pontuacao para a pagina updateScore.php
